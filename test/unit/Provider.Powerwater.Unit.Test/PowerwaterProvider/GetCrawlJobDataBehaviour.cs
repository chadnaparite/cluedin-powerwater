...
{
    public class GetCrawlJobDataBehaviour : PowerwaterProviderTest
    {
        ...
        [Theory]
        [InlineAutoData(PowerwaterConstants.KeyName.ApiKey, nameof(PowerwaterCrawlJobData.ApiKey))]
        ...
        [Theory]
        [InlineAutoData]
        public async Task PowerwaterCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.Powerwater.Core.PowerwaterCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
